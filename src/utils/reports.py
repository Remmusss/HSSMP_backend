...
def get_hr_report_logic(session: Session):
    ...
    status_allocation = (
        session.query(
            HmEmployee.Status, func.count(HmEmployee.EmployeeID).label("employee_count")
        )
        ...
        "department_allocation": [
            {
                ...
                "percentage": (
                    round((emp_count / total_employees * 100), 2)
...
def get_payroll_report_logic(session: Session, month: Optional[date] = None):
    ...
    average_salary = (
        float(total_budget) / total_salary_count if total_salary_count > 0 else 0
    )
        ...
        "department_analysis": [
            {
                ...
                "budget_percentage": (
                    round((float(total_salary) / float(total_budget) * 100), 2)
                    if total_budget > 0
                    else 0
                ),
            }
...
def get_dividend_report_logic(session: Session, year: Optional[int] = None):
    total_dividend_query = session.query(
        func.sum(HmDividend.DividendAmount).label("total_dividend")
    )
    employee_count_query = session.query(
        func.count(func.distinct(HmDividend.EmployeeID))
    )
    ...
    base_query = (
        ...
        .join(
            HmDepartment,
            HmEmployee.DepartmentID == HmDepartment.DepartmentID,
            isouter=True,
        )
    ...
    if year:
        total_dividend_query = total_dividend_query.filter(
            extract("year", HmDividend.DividendDate) == year
        )
        employee_count_query = employee_count_query.filter(
            extract("year", HmDividend.DividendDate) == year
        )
        ...
        "employees_with_shares": [
            {
                ...
                "percentage": (
                    round((float(total_div) / float(total_dividend) * 100), 2)
                    if total_dividend > 0
                    else 0
                ),
            }
