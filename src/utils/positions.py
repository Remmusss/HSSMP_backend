...
def add_and_sync_position(session_human: Session, session_payroll: Session, position: PositionCreate):
    max_id_human = session_human.query(HmPosition.PositionID).order_by(HmPosition.PositionID.desc()).first()
    max_id_payroll = session_payroll.query(PrPosition.PositionID).order_by(PrPosition.PositionID.desc()).first()
    ...
    if session_human.query(HmPosition).filter_by(PositionID=position_id).first():
        raise HTTPException(status_code=400, detail="PositionID đã tồn tại trong HUMAN_2025")
    if session_payroll.query(PrPosition).filter_by(PositionID=position_id).first():
        raise HTTPException(status_code=400, detail="PositionID đã tồn tại trong payroll")
    ...
    try:
        ...
        return {"message": "Chức vụ đã được thêm và đồng bộ hóa thành công", "PositionID": position_id}
...
def update_and_sync_position(session_human: Session, session_payroll: Session, position_id: int, position: PositionUpdate):
    human_position = session_human.query(HmPosition).filter_by(PositionID=position_id).first()
    payroll_position = session_payroll.query(PrPosition).filter_by(PositionID=position_id).first()
    ...
    if not human_position or not payroll_position:
        ...
        raise HTTPException(status_code=404, detail=f"Chức vụ không tồn tại trong: {', '.join(missing)}")
    ...
    return {"message": "Chức vụ đã được cập nhật và đồng bộ hóa thành công", "PositionID": position_id}
            
def delete_and_sync_position(session_human: Session, session_payroll: Session, position_id: int):
    human_position = session_human.query(HmPosition).filter_by(PositionID=position_id).first()
    payroll_position = session_payroll.query(PrPosition).filter_by(PositionID=position_id).first()
    ...
    try:
        human_employees = session_human.query(HmEmployee).filter_by(PositionID=position_id).all()
        payroll_employees = session_payroll.query(PrEmployee).filter_by(PositionID=position_id).all()
        ...
        return {"message": "Chức vụ đã được xóa thành công khỏi cả hai hệ thống."}
